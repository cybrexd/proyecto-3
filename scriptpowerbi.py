...
listcriptos = ["ETH/USD","XRP/USD","DOGE/USD","SOL/USD","LTC/USD","TRX/USD","AAVE/USD","ALGO/USD","AUD/USD"]
market_name = 'BTC/USD'
...
df["cripto"] = 'BTC/USD'
df = df.drop(columns=['startTime', 'time',"open"])

#########
for element in listcriptos:
 market_name = element
 path = f'/markets/{market_name}'
 url = api_url + path
 
 res = requests.get(url).json()
 df1 = pd.DataFrame(res)['result']
 ...
 df1["cripto"] = element
 df1 = df1.drop(columns=['startTime', 'time',"open"])
